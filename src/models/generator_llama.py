...
class Llama3EvalVLM(EvalVLM):
    def __init__(self, cfg: object, model: Any, tokenizer: Any, image_transform: Any):
        ...
        chat_template = "{% if not add_generation_prompt is defined %}{% set add_generation_prompt = false %}{% endif %}{% if not enable_thinking is defined %}{% set enable_thinking = false %}{% endif %}{% for message in messages %}{{'<|start_header_id|>' + message['role'] + '<|end_header_id|>' + '\n' + message['content'] + '<|eot_id|>'}}{% endfor %}{% if add_generation_prompt %}{{ '<|start_header_id|>assistant<|end_header_id|>\n' }}{% endif %}{% if enable_thinking %}{{ '<|think|>\n' }}{% endif %}"
        ...
        if self.cfg.eval_enable_vllm:
            ...
            self.model.decoder.generation_config.pad_token_id = (
    ...
    def get_decoder_state_dict(self):
        ...
        if (
            ...
            # TODO: currently, we only use cfg.llama_model_scale to determine
            # wether to split the lm_head.weight for vllm
            is_sharded_lm_head = float(self.cfg.llama_model_scale) > 3
    ...
    def _construct_model_inputs(self, contexts: List[str], images: List[torch.Tensor]):
        ...
        bot_id = self.tokenizer.convert_tokens_to_ids("<|begin_of_text|>")
        pad_id = self.tokenizer.convert_tokens_to_ids("<|end_of_text|>")
        ...
        for i, conv in enumerate(contexts):
            ...
            if has_img and self.cfg.enable_image_prompt:
            ...
            # NOTE: for debugging
            # usr_text = "Hello, hello!? What is the meaning of life?"
    ...
    def _post_processing_response(self, generated_ids, benchmark, print_info=False):
        ...
        out_strs = [self._decode_text(ids).strip() for ids in generated_ids]
        thinking_traces = [""] * len(out_strs)
